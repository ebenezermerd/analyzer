...
from .issue_analyzer import IssueAnalyzer, _count_code_files, _count_code_python_files, _is_code_file, _body_has_links_or_images, _body_is_pure_and_substantial, compute_passes, pre_filter
...
class InteractiveSession:
    ...
    def __init__(self, token: str | None = None, app_config: AppConfig | None = None, language: str | None = None):
        self.app_config = app_config or AppConfig()
        self.language = language or self.app_config.default_language or "Python"
        ...
        self.analyzer = IssueAnalyzer(self.client, language=self.language)
        ...
        # Settings
        self.min_stars: int = self.app_config.min_stars
        self.max_repos: int = self.app_config.max_repos
        self.max_issues: int = self.app_config.max_issues_per_repo
        self.min_score: float = self.app_config.min_score
        self.concurrency: int = (
            self.app_config.token_concurrency if self.token else self.app_config.no_token_concurrency
        )

        # Profile
        profile_name = self.app_config.default_profile or PR_WRITER_PROFILE.name
        try:
            self.profile: ScoringProfile = load_profile(profile_name)
        except Exception:
    ...
    def _build_prompt(self) -> str:
        lang_short = {"Python": "py", "JavaScript": "js", "TypeScript": "ts"}.get(self.language, self.language)
        base = f"\033[1;36missue-finder\033[0m[\033[0;35m{lang_short}\033[0m]"
    ...
    def _print_banner(self):
        ...
        console.print(Panel(
            ...
            f"Language: [bold]{self.language}[/bold]  [dim](use [bold]lang <name>[/bold] to switch)[/dim]\n"
    ...
    def _cmd_search(self, query: str):
        """Search GitHub for repositories in the selected language (uses web scraping)."""
        if not query:
        ...
        console.print(
            f"[cyan]Scraping GitHub search for {self.language} repos matching "
        ...
        with Progress(
            ...
            try:
                ...
                try:
                    ...
                    search_q = f"{query} language:{self.language} stars:>={self.min_stars} {exclude_clause}"
    ...
    def _cmd_light(self, query: str):
        ...
        console.print(f"[cyan]Searching for lightweight {self.language} repos matching '{escape(query)}'…[/cyan]")
    ...
    def _cmd_best(self, query: str):
        ...
        console.print(f"[cyan]Searching for best {self.language} repos matching '{escape(query)}'…[/cyan]")
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as prog:
        ...
        for i, p in enumerate(parts):
            ...
            if pl == "all":
                ...
                try:
                    ...
                    pass
        self._fetch_and_show_issues(limit, show_all=show_all, top_n=top_n)
    ...
    def _cmd_scan(self, args: str):
        ...
        for r in results:
            ...
            row = {
                ...
                "code_files_changed": r.details.get("code_files_changed", 0),
    ...
    def _cmd_lang(self, args: str):
        """Show or switch target language."""
        if not args:
            console.print(f"[cyan]Current language:[/cyan] [bold]{self.language}[/bold]")
            console.print(f"[dim]Supported: {', '.join(SUPPORTED_LANGUAGES)}[/dim]")
            console.print("[dim]Shortcuts: py, js, ts[/dim]")
            return
        ...
        lang = mapping.get(args.strip().lower())
        if not lang:
            console.print(f"[red]Unknown language: {escape(args)}[/red]")
            console.print(f"[dim]Supported: {', '.join(SUPPORTED_LANGUAGES)} (or py/js/ts)[/dim]")
            return

        self.language = lang
        self.analyzer = IssueAnalyzer(self.client, language=lang)
        # Update profile to match language
        self.profile = self.profile.with_language(lang)
        console.print(f"[green]Switched to [bold]{lang}[/bold] — search, discover, and analysis will target {lang} repos.[/green]")

    # ── SHA Extractor ──────────────────────────────────────────

    def _cmd_sha(self, args: str):
        """Extract base SHA from any issue+PR URL pair."""
        from .config import parse_github_url

        # Parse args or prompt
        if args:
            parts = args.strip().split()
            if len(parts) == 2:
                issue_url, pr_url = parts
            else:
                console.print("[yellow]Usage:[/yellow] sha <issue_url> <pr_url>")
                console.print("[dim]  Example: sha https://github.com/owner/repo/issues/123 https://github.com/owner/repo/pull/456[/dim]")
                return
        else:
            try:
        ...
        if issue_type != "issue":
            console.print(f"[red]Expected an issue URL, got a {issue_type} URL[/red]")
            return
        if pr_type != "pr":
            console.print(f"[red]Expected a PR URL, got a {pr_type} URL[/red]")
            return
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as prog:
            task = prog.add_task(f"Fetching issue #{issue_num} from {repo}...", total=None)

            issue_info = self.client.get_issue_info(repo, issue_num)
            if not issue_info:
                prog.update(task, description=f"[red]Issue #{issue_num} not found[/red]")
                return

            if issue_info.state != "closed":
                console.print(f"[yellow]Warning: Issue #{issue_num} is not closed (state: {issue_info.state})[/yellow]")

            prog.update(task, description=f"Fetching PR #{pr_num}...")

            pr_body = self.client.get_pr_body(repo, pr_num)
            if pr_body:
            ...
            prog.update(task, description="Extracting base SHA...")
            base_sha = self.client.get_pr_base_sha(repo, pr_num)

            if not base_sha:
                prog.update(task, description=f"[red]Could not extract base SHA for PR #{pr_num}[/red]")
                return
        ...
        console.print(Panel(
            ...
            f"[bold]Issue:[/bold]     [cyan]#{issue_num}[/cyan] — {escape(issue_info.title)}\n"
            f"[bold]PR:[/bold]        [cyan]#{pr_num}[/cyan]\n"
            f"[bold]Base SHA:[/bold]  [yellow]{base_sha}[/yellow]",
            title="[bold green]Base SHA Extractor[/bold green]",
            border_style="green",
        ))
        console.print()
    ...
    def _cmd_help(self, _args: str):
        ...
        rows = [
            ("search <keyword>",      "Search GitHub for repos (use 'lang' to change language)"),
            ...
            ("issues [limit]",        "Smart-filtered & ranked closed issues"),
            ("issues all [limit]",    "Show all closed issues (unfiltered)"),
            ("issues top <N>",        "Auto-analyze top N ranked issues"),
            ...
            ("discover trending",     "Only trending repos for current language"),
            ...
            ("lang <name>",           "Switch language (Python/JavaScript/TypeScript/py/js/ts)"),
            ("─── SHA Extractor ───",  "─────────────────────────────────────"),
            ("sha <issue_url> <pr_url>", "Extract base SHA from any issue+PR URLs"),
            ("sha",                    "Prompt for issue URL and PR URL interactively"),
            ("─── Settings ───",       "─────────────────────────────────────"),
    ...
    def _print_repo_panel(self, info: RepoInfo):
        result = analyze_repo(info, language=self.language)
        ok = result.passes
    ...
    def _fetch_and_show_issues(self, limit: int, *, show_all: bool = False, top_n: int = 0):
        console.print(
            f"[cyan]Fetching closed issues from {self.selected_repo.full_name} (limit {limit})…[/cyan]"
        )
        self.issues_cache = []
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as prog:
            task = prog.add_task("Fetching closed issues via API…", total=None)
            max_pages = max(1, limit // 25 + 1)
            try:
                client = self._get_async_client()
                self.issues_cache = self._run_async(
                    client.list_closed_issues(self.selected_repo.full_name, max_pages=max_pages, max_issues=limit)
                )
                prog.update(task, description=f"Found {len(self.issues_cache)} issues.")
            except Exception:
                prog.update(task, description="API failed, trying scraper…")
                try:
                    ...
                    prog.update(task, description="Scraper failed, trying PyGithub…")
        ...
        if not show_all and metrics:
            ...
            self.issues_cache = filtered
            if hidden_count:
                console.print(f"[dim]  ({hidden_count} issues filtered out — use [bold]issues all[/bold] to see everything)[/dim]")
        ...
        if metrics:
            ...
            def _relevance(iss):
                ...
                if m.get("body_pure"):
                    s += 2.0
                py = m.get("py_files")
                if py is not None and py >= min_files:
                    s += 3.0
                elif py is not None:
                    s += py * 0.5
                if any(l.lower() in preferred for l in iss.labels):
                    s += 1.0
                return -s

            self.issues_cache.sort(key=_relevance)

        if not self.issues_cache:
            console.print(
                "[yellow]No issues match the current profile criteria.[/yellow]\n"
                "[dim]Use [bold]issues all[/bold] to see unfiltered results.[/dim]"
            )
            return

        self._display_issues_table()

        # ── Auto-analyze top N if requested ──────────────────────
        if top_n > 0:
            console.print(f"\n[cyan]Auto-analyzing top {top_n} issues…[/cyan]")
            for iss in self.issues_cache[:top_n]:
                try:
                    self._analyze_issue(iss.number)
                    console.print()
                except KeyboardInterrupt:
    ...
    async def _enrich_one(self, client, repo: str, iss: IssueInfo):
        ...
        try:
            ...
            # Check if any PR actually closes this issue (not just references it)
            closing_pr = None
            if pr_nums:
                for pn in pr_nums:
                    pr = await client.get_pr_detail(repo, pn)
                    if pr and iss.number in pr.closes_issues:
                        closing_pr = pr
                        break
                # If no closing PR found, use first PR for file count only
                if not closing_pr:
                    pr = await client.get_pr_detail(repo, pr_nums[0])
                else:
                    pr = closing_pr

            metrics["has_pr"] = closing_pr is not None
            if pr_nums and not closing_pr:
                metrics["has_pr"] = None  # linked but doesn't close → show as "?"

            # Get code file count from the best PR
            if pr_nums and pr and pr.files:
                metrics["py_files"] = _count_code_files(pr.files, self.language)
            ...
            metrics["body_pure"] = _body_is_pure_and_substantial(iss.body)
    ...
    def _display_issues_table(self):
        ...
        tbl.add_column("Rel", justify="center", width=5)
        tbl.add_column("PR?", justify="center", width=5)
        tbl.add_column(f"Code({min_files}+)", justify="center", width=7)
        ...
        for iss in self.issues_cache:
            ...
            if any(l.lower() in preferred for l in iss.labels):
                rel += 1.0
            rel_str = f"[green]{rel:.0f}[/green]" if rel >= 6 else (f"[yellow]{rel:.0f}[/yellow]" if rel >= 3 else f"[red]{rel:.0f}[/red]")
        ...
        # Legend
        console.print(
            f"[dim]Columns: Rel = relevance score · PR? = linked PR · Code({min_files}+) = code files "
            f"· Body = pure text · Filter = pre-filter[/dim]"
        )
        console.print(
            "[dim]Sorted by relevance. Use [bold]analyze <issue#>[/bold] for full analysis. "
            "[bold]issues all[/bold] = unfiltered · [bold]issues top 5[/bold] = auto-analyze top N.[/dim]"
        )
    ...
    def _analyze_issue(self, num: int):
        from .issue_analyzer import (
            IssueAnalysisResult, _is_code_file, _is_test_file,
        ...
        console.print(f"[cyan]Analyzing #{num} (async API + Timeline)…[/cyan]")

        # Use the async client (same path as enrichment) for consistency
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as prog:
            task = prog.add_task("Finding linked PRs via Timeline API…", total=None)
            client = self._get_async_client()

            # Try async client first (consistent with enrichment), fall back to scraper
            try:
                pr_nums = self._run_async(client.get_linked_prs(repo, num))
            except Exception:
            ...
            if pr_nums:
                ...
                for pr_num in pr_nums:
                    try:
                        pr = self._run_async(client.get_pr_detail(repo, pr_num))
                    except Exception:
                ...
                if not best_pr:
                    for pr_num in pr_nums:
                        ...
                        if pr and num in pr.closes_issues:
                            best_pr = pr
                            break
                # No unconditional fallback — only use PRs that close this issue
            prog.update(task, description="Done.")
        ...
        # Body check
        body_pure = _body_is_pure_and_substantial(issue_info.body)
        if body_pure:
        ...
        if best_pr and best_pr.files:
            code_files = _count_code_files(best_pr.files, self.language)
            subst = _has_substantial_changes(best_pr.files, self.profile.min_substantial_changes, self.language)
        ...
        if code_files >= self.profile.min_code_files_changed:
            ...
            reasons.append(f"{code_files} code files changed")
        else:
            reasons.append(f"Only {code_files} code files changed (need >= {self.profile.min_code_files_changed})")
        ...
        # Label boost
        if issue_info.labels and hasattr(self.profile, "label_boost_score"):
            pref = {l.lower() for l in self.profile.preferred_labels}
            matched = [l for l in issue_info.labels if l.lower() in pref]
            if matched:
                score += self.profile.label_boost_score
                reasons.append(f"Has preferred label: {', '.join(matched)}")

        # Recency boost
        if issue_info.closed_at and hasattr(self.profile, "recency_boost_score"):
            from datetime import datetime, timezone
            try:
                closed = datetime.fromisoformat(issue_info.closed_at.replace("Z", "+00:00"))
                age_months = (datetime.now(timezone.utc) - closed).days / 30.0
                if age_months <= self.profile.recency_months:
                    score += self.profile.recency_boost_score
        ...
        passes = (
            best_pr is not None
            and compute_passes(
                score, code_files, best_pr.files if best_pr else [],
                issue_info.body, self.profile,
            )
        )
        ...
        # Score: code files
        code_files = analysis.details.get("code_files_changed", 0)
        min_files = self.profile.min_code_files_changed
        files_pass = code_files >= min_files
        score_tbl.add_row(
            f"code files changed (>= {min_files})",
        ...
        if analysis.pr_analysis:
            ...
            code_files_count = _count_code_files(pr.files, self.language)
            test_files = [f for f in pr.files if _is_test_file(f.filename)]
            doc_files = [f for f in pr.files if _is_doc_file(f.filename)]
            code_file_list = [f for f in pr.files if _is_code_file(f.filename, self.language)]
            ...
            if pr.files:
                ...
                for f in pr.files:
                    if _is_code_file(f.filename, self.language):
        ...
        row = {
            ...
            "code_files_changed": analysis.details.get("code_files_changed", 0),
    ...
    def _resolve_issue(self, num: int) -> IssueInfo | None:
        ...
        # Try async API first (consistent with enrichment)
        console.print(f"[dim]Fetching issue #{num} via API…[/dim]")
        try:
            client = self._get_async_client()
            detail = self._run_async(client.get_issue_detail_api(self.selected_repo.full_name, num))
            if detail:
                self.issues_cache.append(detail)
                return detail
        except Exception:
            pass

        console.print(f"[dim]Falling back to scraper for #{num}…[/dim]")
        try:
...
def run_interactive(token: str | None = None, app_config: AppConfig | None = None, language: str | None = None) -> int:
    session = InteractiveSession(token, app_config=app_config, language=language)
    return session.run()
