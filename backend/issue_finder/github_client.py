...
class GitHubClient:
    ...
    def search_repos_by_language(
        ...
        """Search for repositories matching criteria in the given language."""
        exclude = exclude_words or ["collection", "list", "guide", "projects", "exercises"]
        exclude_query = " ".join(f"NOT {w}" for w in exclude)
        query = f"language:{language} stars:>={min_stars} {exclude_query}"
    ...
    def search_python_repos(
        ...
        """Legacy alias: search for Python repos."""
        return self.search_repos_by_language("Python", min_stars, exclude_words, max_results)
    ...
    def get_issue_info(self, full_name: str, issue_number: int) -> IssueInfo | None:
