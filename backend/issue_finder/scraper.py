...
from .config import get_heavy_deps

HEAVY_DEPS = get_heavy_deps("Python")  # default for backward compat
...
class GitHubScraper:
    ...
    def search_light_repos(
        ...
        """Search for lightweight repos (small size, no heavy deps)."""
        q = f"{query} language:{language} stars:>={min_stars} size:<50000"
        ...
        for item in items:
            ...
            heavy = get_heavy_deps(language)
            if any(dep in combined for dep in heavy):
                continue
            results.append(RepoInfo(
                full_name=repo_name,
                stars=item.get("followers", 0),
    ...
    def search_best_repos(
        ...
        q = f"{query} language:{language} stars:>={min_stars} pushed:>2024-06-01 good-first-issues:>2"
